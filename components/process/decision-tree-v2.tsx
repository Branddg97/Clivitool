...
export function DecisionTreeV2({ processId }: DecisionTreeProps) {
  ...
  // Un paso es final si no tiene nextStep ni opciones
  const isLastStep = !currentStep?.nextStep && !currentStep?.options

  console.log('DecisionTreeV2 Debug:', {
    processId,
  ...
  const handlePrevious = () => {
    if (stepHistory.length > 1) {
      ...
      newHistory.pop() // Eliminar el paso actual
      const previousIndex = newHistory[newHistory.length - 1]
      setCurrentStepIndex(previousIndex)
      setStepHistory(newHistory)
      
      // Remover el paso actual de completados
  ...
  const handleFinish = () => {
    ...
    const timeElapsed = (endTime.getTime() - startTime.getTime()) / (1000 * 60) // en minutos

    // Guardar estadísticas
    const processStats = {
      processId,
      processTitle: processList[processId]?.[0]?.title || "Proceso",
  ...
  const getStepTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "question": return <AlertTriangle className="h-5 w-5 text-orange-600" />
      case "action": return <User className="h-5 w-5 text-purple-600" />
      case "validation": return <FileText className="h-5 w-5 text-green-600" />
  ...
  const getStepTypeBadge = (type: string) => {
    switch (type) {
      ...
      case "question": return <Badge className="bg-orange-100 text-orange-800">Decisión</Badge>
      case "action": return <Badge className="bg-purple-100 text-purple-800">Acción</Badge>
      case "validation": return <Badge className="bg-green-100 text-green-800">Validación</Badge>
      default: return <Badge>Paso</Badge>
    }
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="border-2 border-purple-200">
        ...
        <CardContent className="space-y-4">
          ...
          {/* Navigation - SIEMPRE mostrar botón para pasos sin opciones */}
          <div className="flex items-center justify-between pt-4 border-t">
            ...
            {/* Mostrar botón SIEMPRE que no sea question con opciones */}
            {!currentStep.options && (
              <Button 
